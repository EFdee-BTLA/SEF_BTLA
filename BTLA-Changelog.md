...
Buffed grenades damage (SEF values)
Added LAPD SWAT Uniform (Replaced NYPD patch and some other adjustments)
Level III armor now uses Steel Plates instead of Ceramic Plates (cannot be shredded anymore) 
Changed max loadout weight to 30 kgs
Improved recoil when firing on full auto
Reduced tased effect duration on players from 15 seconds to 4 seconds (AI time)
Reduced pepper spray effect duration from 12 seconds to 6 seconds
Added new meshes for most guns (thanks Beppe for this)
Added Remington M870 to the game (thanks Beppe for this)
Reverted to SEF AI
