...
simulated function float GetTotalWeight() {
  ...
  local class KevlarNightVisionClass; // special case
  ...
  for(i = 0; i < Pocket.EnumCount; i++) {
    ...
    switch(pock) {
      ...
      case Pocket_HeadArmor:
		...
		case Pocket_HeadArmor:
        KevlarNightVisionClass = class(DynamicLoadObject("SwatEquipment.KevlarNVGogglesBase",class'class'));
        if(ClassIsChildOf(LoadOutSpec[i], KevlarNightVisionClass)) {
          total += 2.04; // IMPORTANT: Make sure this matches the value in KevlarNVGogglesBase.uc !!!
        } else {
          HeadgearClass = class<Headgear>(LoadOutSpec[i]);
          total += HeadgearClass.default.Weight;
        }
  ...
  for(i = 0; i < Pocket.EnumCount; i++) {
    ...
    switch(pock) {
	  ...
	  case Pocket_HeadArmor:
        KevlarNightVisionClass = class(DynamicLoadObject("SwatEquipment.KevlarNVGogglesBase",class'class'));
        if(ClassIsChildOf(LoadOutSpec[i], KevlarNightVisionClass)) {
          total += 2.644; // IMPORTANT: Make sure this matches the value in KevlarNVGogglesBase.uc !!!
        } else {
          HeadgearClass = class<Headgear>(LoadOutSpec[i]);
          total += HeadgearClass.default.Bulk;
        }
